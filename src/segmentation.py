...
class SatelliteSegmentation(SatelliteComponent):
    """
    ...
    def parse_options(self, params):
        import argparse
        parser = argparse.ArgumentParser(prog='SatelliteSegmentation', add_help=False)
    ...
    def show(self, viewer, ann, pred):
        ...
        for satellite_img in pred.images:
            ...
            # from satellite_framework.src.utils import contours2mask
            ...
            # for satellite_obj in ann.images[0].objects:
            #     for contour in satellite_obj.multipolygon:
            #         ann_contours.append(contour)
            #     for category in satellite_obj.categories.get_labels():
            ...
            # for satellite_obj in pred.images[0].objects:
            #     for contour in satellite_obj.multipolygon:
    ...
    def evaluate(self, fs, ann, pred):
        from satellite_framework.src.utils import numpy2geometry
        ...
        for satellite_img in pred.images:
            ...
            for obj_idx, obj_val in enumerate([ann.images[image_idx].objects, satellite_img.objects]):
                for satellite_obj in obj_val:
                    fs.write(';' + str(satellite_obj.id) + ';' + str(len(satellite_obj.multipolygon)))
                    for contour in satellite_obj.multipolygon:
                        fs.write(';' + str(numpy2geometry(contour)))
                    fs.write(';' + str(len(satellite_obj.categories.get_labels())))
                    for label in satellite_obj.categories.get_labels():
    ...
    def save(self, dirname, pred):
        ...
        for img in pred.images:
            ...
            output_json['images'].append(dict({'id': 0, 'file_name': os.path.basename(img.filename), 'width': int(img.tile[2]-img.tile[0]), 'height': int(img.tile[3]-img.tile[1]), 'date_captured': img.timestamp}))
            for idx, obj in enumerate(img.objects):
                ...
                output_json['annotations'].append(dict({'id': idx+1, 'image_id': 0, 'category_id': int(categories.index(obj.categories.get_labels()[-1])+1), 'segmentation': multipolygon, 'area': float(np.sum(areas)), 'bbox': list(map(int, [tl[0], tl[1], br[0]-tl[0], br[1]-tl[1]])), 'iscrowd': int(len(img.objects) > 1)}))
            for idx, label in enumerate(categories):
                output_json['categories'].append(dict({'id': idx+1, 'name': label.name, 'supercategory': ''}))
            # Save COCO annotation file
            root, extension = os.path.splitext(img.filename)
